  ...
  try {
    ...
    if (!unitId) {
      return NextResponse.json(
        {
          error:
            "Please choose one of our suites before sending your inquiry.",
        },
        {
          status: 400,
        }
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Your message should be between 10 and 2,000 characters.",
        },
        {
          status: 400,
        }
    ...
    if (
      ...
      return NextResponse.json(
        {
          error: `${unit.shortName} accommodates 1–${unit.maxGuests} guests.`,
        },
        {
          status: 400,
        }
    ...
    if (body.consent !== true) {
      return NextResponse.json(
        {
          error:
            "Please agree to the use of your details to respond to your inquiry.",
        },
        {
          status: 400,
        }
    ...
    if (hasDates) {
      if (
        !isDateKey(body.checkIn) ||
        !isDateKey(body.checkOut)
      ) {
        return NextResponse.json(
          {
            error:
      ...
      if (!unitAvailability) {
        return NextResponse.json(
          {
            error:
              "We couldn't check the availability of this suite right now.",
          },
          {
            status: 503,
          }
      ...
      if (
        body.checkIn < availability.today ||
        body.checkOut <= body.checkIn ||
        body.checkOut > availability.maxDate
      ) {
        return NextResponse.json(
          {
            error:
              "Please choose future dates within the next year, with check-out after check-in.",
          },
          {
            status: 400,
          }
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              `${unit.shortName} accepts stays from ${unit.minNights} to ${unit.maxNights} nights. For longer visits, please send a general inquiry.`,
          },
          {
            status: 400,
          }
      ...
      if (
        nights.some((date) =>
          unitAvailability.blockedDates.includes(
            date
          )
        )
      ) {
        return NextResponse.json(
          {
            error:
              `Some of those dates are reserved for ${unit.shortName}. Please refresh the calendar or choose the other suite.`,
          },
          {
            status: 409,
          }
       ...
       * - +₱300 per additional guest per night
       * - Maximum 4 guests
       * - No cleaning fee
       */
      estimatedTotal = getQuote(
        unitId,
        body.checkIn,
        body.checkOut,
        guests
    ...
    if (recent.length >= 5) {
      return NextResponse.json(
        {
          error:
            "You’ve sent a few inquiries recently. Please try again in 15 minutes.",
        },
        {
          status: 429,
        }
    ...
    return NextResponse.json(
      {
        error:
          "We couldn’t save your inquiry right now. Please try again shortly.",
      },
      {
        status: 503,
      }
